      ...
      if (action_considered.sf != NULL)
	{
	  for(i = 0;i < action_considered.sf->num_PC_overall; i++)
	      ...
	      if (action_considered.sf->PC_overall[i]<0)
		{
		  ...
		  insert_bit_in_bit_table(Numeric.num_Pc_ef_matrix, (- (action_considered.sf->PC_overall[i])), action_number);
		  if(!GET_BIT(num_fact_vector, (-action_considered.sf->PC_overall[i])))
		    num_unsatisfied_num_precond++;
		}
	  ...
	  for(i = 0;i < action_considered.sf->num_PC_end; i++)
	      ...
	      if (action_considered.sf->PC_end[i]<0)
		{
		  ...
		  insert_bit_in_bit_table(Numeric.num_Pc_ef_matrix, (- (action_considered.sf->PC_end[i])), action_number);
		  if(!GET_BIT(num_fact_vector, (-action_considered.sf->PC_end[i])))
		    num_unsatisfied_num_precond++;
		}
