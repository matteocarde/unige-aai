...
/*
-----------------------------------------------------------------
	DESCRIPTION	: Verifica se un fatto � presente nella
			  lista dei fatti
...
check_action_f ()
{
  ...
  for(level=0; level < GpG.curr_plan_length; level++)
      ...
      for(i=0; i< level; i++)
	{
      ...
      if (fabs(max_time + get_action_time (posA, level) - GET_ACTION_OF_LEVEL(level)->time_f) < GpG.min_delta_time )
	{
	  if (savelev >0)
	    {
  ...
  for (level = 0; level < GpG.curr_plan_length; level++)
      ...
      if (ind >= 0)
	{
	  ...
	  if ((TempAct = GET_ACTION_OF_LEVEL (level))->w_is_used)
		...
		if(TempAct->time_f - get_action_time (ind, level) - TempAct->action_f->time_f > GpG.time_tolerance || -(TempAct->time_f - get_action_time (ind, level) - TempAct->action_f->time_f) > GpG.time_tolerance)
		  {
	  ...
	  for (indPrec = 0; indPrec < gef_conn[ind].num_PC; indPrec++)
		...
		get_action_time (ind, level) - GpG.time_tolerance)
	      {
  ...
  for (i = 0; i < GpG.curr_plan_length; i++) {
	...
	if (infAction -> PC_interval[j] >= 0) {
	  ...
	  if (k == timedFct -> num_act_PC)
	    {
	      printf("\nError TL3 : l'azione %s � associata a  ", print_op_name_string(infAction -> position, temp_name));
