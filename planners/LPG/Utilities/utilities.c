...
int is_plan_better ()
{
  ...
  **/
  ...
  /**
     Se il piano attuale migliora di GpG.time_tolerance il best_plan allora il best_plan e' quello attuale
     **
  ...
  **/
  ...
  if ((GpG.best_cost - GpG.time_tolerance) > GpG.total_cost_from_metric)
    {
      if (DEBUG3)
	printf ("\n\n !!!  Find BETTER solution => SAVE \n");
      return TRUE;
    ...
    {
      if ((GpG.best_cost - GpG.time_tolerance) < GpG.total_cost_from_metric &&
	  (GpG.total_cost_from_metric - GpG.time_tolerance) < GpG.best_cost)
	...
	**
      ...
      if (act_pos != -1)
	{
	  ...
	  **/
	  ...
	  GpG.total_cost+= get_action_cost(act_pos,0,NULL);
	  /* ADAPT  */
...
int load_pddl2_plan (char *plan_file, PlanAction ** plan_actions, int start_level)
{
  ...
  **/
  ...
  GpG.total_time=GpG.input_plan_end_time;
...
void initialize_preset_values (void)
{
  ...
  */
  ...
  GpG.incremental_goal_resolution=FALSE;
  ...
  for (temp = 0, i = 0; i < size; i++)
	...
	/** 
	    ...
	    **
	    ...
	    It increases temp
	**/
        mutex=  vectlevel[level]->prec_vect[i] & (vectlevel[level]->fact_vect[i]) & CONVERT_ACTION_TO_VERTEX (act_pos)->ft_exclusive_vect[i];
	if(mutex)
	  temp +=  count_bit1 (mutex);
...
Bool process_command_line (int argc, char *argv[])
{
  ...
  int i, heuristic;
  float noise;
  int  set_max_cputime_for_local_search=0;
  ...
  GpG.min_delta_time=MIN_DELTA_TIME;
  GpG.time_tolerance=TIME_TOLERANCE;
  ...
  for (i = 1; i < argc; i++)
      ...
      else if (strcmp (argv[i], "-cputime") == 0)
	{
	  ...
	  if(set_max_cputime_for_local_search==0)
	    GpG.max_cputime_for_local_search=GpG.max_cputime;
	}

      else if (strcmp (argv[i], "-cputime_localsearch") == 0){
        set_max_cputime_for_local_search=1;
      ...
      else if (strcmp (argv[i], "-v") == 0)
	{
	  ...
	  if (strcmp (temp, "off") == 0)
	    GpG.verbose = FALSE;
    	  ...
    	  sscanf (argv[++i], "%lf", &GpG.min_delta_time);
    	  GpG.time_tolerance=(GpG.min_delta_time/2.0);
      }
	...
	**/
	...
	GpG.disable_pruning = TRUE;
      else if (strcmp (argv[i], "-incremental_goal_resolution") == 0)
	GpG.incremental_goal_resolution=TRUE;

     else if (strcmp (argv[i], "-force_neighbour_insertion") == 0)
	GpG.force_neighbour_insertion=TRUE;

#ifdef __cplusplus    
      else if (strcmp (argv[i], "-get_peak_memory") == 0)
	GpG.get_peak_memory=TRUE;
#endif
...
void print_act_eff_prec(int action, int level)
{
 ...
 if (gef_conn[action].sf != NULL)
      ...
      for (i = 0; i < gef_conn[action].sf->num_A_start; i++)
	{
	  ...
	  if(el<0)
	    {
	      char *str=print_numeric_ft_string(-el, temp_name, gcomp_var_effects);
	      printf("\n Numeric  Effect at start  %s ", str );
       }
	  else
	    {
