...
void print_parser_info_for_debug()
{
  ...
  if (gcmd_line.display_info == 142)
      ...
      for (i = 0; i < gnum_op_conn; i++)
	{
	  ...
	  printf ("\nSIZE = %ld",
		 sizeof (gop_conn[i]) + sizeof (int) * (gop_conn[i].num_E - 1));
	}
...
print_op_name_string (int pos, char *out_string)
{
 ...
 if (pos < 0 )
  ...
  if(DEBUG2)
    {
      sprintf(out_string+strlen(out_string), "[#%d]",pos);
...
print_ft_name_string (int pos, char *out_string)
{
  ...
  if (pos < 0)
     {
       print_complete_cvar_string(-pos, out_string);
...
void store_pop(char *fact_file_name)
{
   ...
   if (GpG.out_file_name)
	  ...
	  sprintf (cNameFile, "%s/plan_%s_%d.SOL",  dirname(fact_file_name),basename(fact_file_name),  num_plan);
  ...
  */
  for (ind = 0; ind < GpG.curr_plan_length; ind++)
	 ...
	 **
	 ...
	 its time
      **/
      //      propagation_time (GET_ACTION_OF_LEVEL (ind),GpG.time_tolerance);
       propagation_time (GET_ACTION_OF_LEVEL (ind),GpG.min_delta_time);
    ...
    // Print the preconds and effects of the action in xml format

	  
void store_state_metada( FILE *fp,State state , char *msg)
{
  int j;
  int posB, ordB;
  int predecessors=0;
  ...
  for (int i = 0; i < state.num_F; i++ ) {
    fprintf (fp, "<Fact>%s</Fact>",print_ft_name_string(state.F[i],temp_name));
...
void store_action_metada( FILE *fp, ActNode_list infAction, float start_act_time)
{
  int j;
  int posB, ordB;
  int predecessors=0;
  int levB;
  State *S= new_State(max_state_facts);
  int level= *infAction->level;
 ...
 if(predecessors)
   {
       ...
       if ((posB = GET_ACTION_OF_LEVEL (levB)->position) >= 0)
	 {
	   ...
	   if(ordB!=0){
	     fprintf (fp, "<Predecessor><Id>%d</Id>", (levB+1));
	     //Search for Causal Links
	     for(int i=0; i< gef_conn[posB].num_A;i++)
	       for(int j=0;j< gef_conn[infAction->position].num_PC;j++)
		 if(gef_conn[posB].A[i]==gef_conn[infAction->position].PC[j])
		   fprintf (fp, "<CL>%s</CL>",print_ft_name_string(gef_conn[posB].A[i],temp_name));
 ...
 for( levB= *infAction->level+1;levB<GpG.curr_plan_length ; levB++)
   if ((posB = GET_ACTION_OF_LEVEL (levB)->position) >= 0)
     {
       //	ordB=Econstraint_type(posB,levB,infAction->position, *infAction->level);
       ordB=get_constraint_type(infAction->position, *infAction->level, posB,levB);
       if(ordB!=0)
	 {
	   predecessors++;
	   break;
 ...
 if(predecessors)
   {
        fprintf (fp, "<NotOrdered>");
     
	for( levB= *infAction->level-1; levB>=0; levB--)
          if ((posB = GET_ACTION_OF_LEVEL (levB)->position) >= 0)
	  {
	   // ordB=Econstraint_type(posB,levB,infAction->position, *infAction->level);
	   ordB=get_constraint_type(posB,levB,infAction->position, *infAction->level);
 ...
 ef = &gef_conn[infAction->position];
 
 fprintf (fp,"<Preconds>");
 for (j = 0; j < ef->num_PC; j++) {
   fprintf (fp,"<Prec>");
   str = print_ft_name_string(ef->PC[j], temp_name);
   fprintf(fp, "%s", str);
   fprintf (fp,"</Prec>");
 }
 fprintf (fp,"</Preconds>");
 if (ef->sf) {
   if(ef->sf->num_PC_overall>0)
     {
       fprintf (fp,"<OverallPreconds>");
       
       for (j = 0; j < ef->sf->num_PC_overall; j++) {
	 fprintf (fp,"<Prec>");
	 str = print_ft_name_string(ef->sf->PC_overall[j], temp_name);
	 fprintf(fp, "%s", str);
	 fprintf (fp,"</Prec>");
       }
       fprintf (fp,"</OverallPreconds>");
     }
   if(ef->sf->num_PC_end>0){
     fprintf (fp,"<EndPreconds>");
     for (j = 0; j < ef->sf->num_PC_end; j++) {
       fprintf (fp,"<Prec>");
       str = print_ft_name_string(ef->sf->PC_end[j], temp_name);
       fprintf(fp, "%s", str);
       fprintf (fp,"</Prec>");
   ...
   if(ef->sf->num_A_start>0){
     fprintf (fp,"<StartPositiveEffects>");
     for (j = 0; j < ef->sf->num_A_start; j++) {
       fprintf (fp,"<Eff>");
       str = print_ft_name_string(ef->sf->A_start[j], temp_name);
       fprintf(fp, "%s", str);
       fprintf (fp,"</Eff>");      
     }
   ...
   if(ef->sf->num_D_start>0){
     fprintf (fp,"<StartNegativeEffects>");
     
     for (j = 0; j < ef->sf->num_D_start; j++) {
       fprintf (fp,"<Eff>");
       str = print_ft_name_string(ef->sf->D_start[j], temp_name);
       fprintf(fp, "%s", str);
       fprintf (fp,"</Eff>");
 ...
 fprintf (fp,"<PositiveEffects>");
 for (j = 0; j < ef->num_A; j++) {
   fprintf (fp,"<Eff>");
 
   str = print_ft_name_string(ef->A[j], temp_name);
 ...
 fprintf (fp,"<NegativeEffects>");
 for (j = 0; j < ef->num_D; j++) {
   fprintf (fp,"<Eff>");
   str = print_ft_name_string(ef->D[j], temp_name);
   fprintf(fp, "%s", str);
   fprintf (fp,"</Eff>");
 }
 fprintf (fp,"</NegativeEffects>");

 level++; // consider the effects of the action
 
 S->num_F=0; 
 for (int i = 0; i <= gnum_ft_conn; i++)
   if(vectlevel[level]->fact[i].w_is_true != 0)
      S->F[S->num_F++]=i;

  store_state_metada(fp, *S, "State");     
    
  int h_ff= get_1P( S, &ggoal_state );
  fprintf (fp, "<FFheuristic>%d<FFheuristic>",h_ff);

  /*  
  double h_lpg= evaluate_level_for_goals( level,  &ggoal_state );
  fprintf (fp, "<LPGheuristic>%.2f<LPGheuristic>",h_lpg);
  */
 fprintf (fp, "</Action>)");
...
store_temporal_plan (int levels, char *fact_file_name, double time)
{
  ...
  if (GpG.save_quality_plan_with_different_name == 1)
	...
	{
#ifndef __WINLPG__
	  sprintf (cNameFile, "%s/qualityplan_%s_1.SOL", dirname(fact_file_name),basename(fact_file_name));
#else
  ...
  if (GpG.out_file_name)
	...
	{
#ifndef __WINLPG__
	  sprintf (cNameFile, "%s/plan_%s.SOL", dirname(fact_file_name),basename(fact_file_name));
#else
  ...
  //  fprintf (fp, "\nTime %d\n\n", (int) (gtotal_time * 1000.0 + 0.5));
	...
	{
	  fprintf (fp, ";;(:metadata");
	  store_state_metada( fp,ginitial_state, "Init");
	  fprintf (fp, ")");
	}
  ...
  if (time >= 0.0)
      ...
      for (j = 0, i = 0; i < levels; i++)
	{
	  ...
	  if (GET_ACTION_POSITION_OF_LEVEL (i) >= 0)
		...
		start_act_time = (double)(((double) ltime) / 10000.0) + GpG.min_delta_time * gap; //curr_plan_length;

		//  start_act_time =   ROUND_T0_1_1000(infAction->time_f - get_action_time (infAction->position, *infAction->level))  + GpG.min_delta_time * curr_plan_length;
	...
	//	if (DEBUG2)
		...
		delta=start_act_time - prev_start_act_time;
	      ...
	      if( fabsf(delta) < GpG.min_delta_time)
		start_act_time+= GpG.min_delta_time;
	      ...
	      else
		{
		 ...
		 fprintf (fp, " ;; InputAct ");
	      ...
	      if(GpG.print_xml_solution == 2 )
		{
		  store_action_metada( fp, infAction, start_act_time);
		}
	...
	{
	  fprintf (fp, ";;(:metadata");
	  store_state_metada( fp,ggoal_state, "Goals");
	  fprintf (fp, ")");
	}
...
store_temporal_plan_for_quality_mode (char *fact_file_name, double time)
{
  ...
  if (GpG.out_file_name)
	...
	{
#ifndef __WINLPG__
	  sprintf (cNameFile, "%s%plan_%s_1.SOL",  dirname(fact_file_name),basename(fact_file_name)); // TOGLIERE _1
#else
	...
	if (GpG.temporal_plan)
	  {
	    start_time =
	      infAction->time_f - get_action_time (infAction->position,
						   *infAction->level) +
	      START_TIME + GpG.min_delta_time * i;
...
void store_plan(double time)
{
  ...
  if(GpG.pop)
    ...
    {
      if(GpG.durative_actions_in_domain || GpG.print_xml_solution)
	{
#ifndef __ONLY_ONE_PLANNER__
	  if (GpG.mode != QUALITY)
#else
  ...
  for (i = 0; i < levels; i++)
	{
	  ...
	  if (GET_ACTION_POSITION_OF_LEVEL (i) >= 0)
		...
		continue;
	      ...
	      start_time = ROUND_TO_1_1000(infAction->time_f - get_action_time (infAction->position, *infAction->level)) + GpG.min_delta_time * curr_plan_length;
...
char *print_complete_cvar_string (int cv_index, char *str)
{
  GpG.print_complete_cvar_string=1;
  
  print_cvar_string (cv_index, str);

  GpG.print_complete_cvar_string=0;
  return str;
...
char *print_cvar_string (int cv_index, char *str)
{
  ...
  if (cv_index < 0)
    return str;
  cv = &gcomp_var[cv_index];
  l_operator=cv->l_operator;
  if(GpG.print_complete_cvar_string==1 && l_operator==FIX_NUMBER && cv->first_op>0 && cv->orig_operator>0 )
    l_operator=cv->orig_operator;
