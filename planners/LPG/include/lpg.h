 ...
 **/
 ...
 #define ROUND_TO_1_1000(f) ((float)((long)((f)*10000.0 + 0.5 ))/10000.0)
//#define ROUND_TO_1_10000(f) ((float)((long)((f)*10000.0 + 0.5 ))/10000.0)

#else
  
 #define ROUND_TO_1_1000(f) ((float)((long)((f)*10.0 + 0.5 ))/10.0)
#endif
...
{
  OPERATOR_TYPE l_operator, orig_operator;
  int first_op;
  int second_op;
...
typedef struct _GPG
  ...
  /* ADAPT */
    ...
    int last_goal_max_action_level;
    int incremental_goal_resolution;
    int force_neighbour_insertion;
    int print_xml_solution; /* if TRUE print the output plan in XML format; if ==1 add xml code to the solution as a comment */


    int print_complete_cvar_string;
