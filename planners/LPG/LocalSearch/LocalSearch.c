...
void insert_els_in_neighborhood (IntList * ilist, action_set * neighbors)
{
  ...
  for (el = ilist; el; el = el->next)
	...
	{
	  if (gef_conn[el->item].level < 0 && GpG.force_neighbour_insertion!=TRUE)
	    {
	      if (DEBUG2)
		{	
		  printf("\nWarning: trying to insert a non reachable action in neighborhood!"); 
		  print_op_name_string(el->item, temp_name);
		}
...
void update_precond()
{
  ...
  **/
  for (false_fa_pos = 0; false_fa_pos < GpG.num_false_fa; false_fa_pos++) {
	 ...
	 /**
	    Se la precondizione e' nell'ultimo livello, significa che e' una precondizione di un goal,quindi 
	 ...
	 **/
	 ...
	 /**
	    ...
	    aggiornato il moltplicatore dell'azione corrispondente a quel livello, sempre se tale azione e' 
	    in uso
	    **
	 ...
	 **/
         if (GET_ACTION_OF_LEVEL (*inconsistenza->level)->w_is_used){ 
               ...
               /**
		  *inconsistenza->level e' il livello nel quale la precondizione non e' supportata
		  inconsistenza->action e' l'azione che ha tale precondizione
		  **
  ...
  for (false_fa_pos = 0; false_fa_pos < GpG.num_false_fa; false_fa_pos++){
	 ...
	 **
      ...
      **/
      ...
      /**
	 Nel caso in cui la precondizione si trovi in un livello sottostante a quello dei goals viene 
	 aggiornato il moltplicatore dell'azione corrispondente a quel livello, sempre se tale azione e' in 
	 uso
	 **
      ...
      **/
      if (GET_ACTION_OF_LEVEL (*inconsistenza->level)->w_is_used){ //controlla se l'azione che richiede la precondizione e' in uso nel piano
  ...
  **/
  ...
  for (next_level = level - 1; next_level >= 0; next_level--)
	...
	break;
      ...
      if(GpG.incremental_goal_resolution==TRUE && GpG.num_solutions<=0 && next_level<=GpG.last_goal_max_action_level)
	break;
  ...
  **/
  ...
  switch (gcomp_var[index].l_operator)
    ...
    case EQUAL_OP:
      ...
      // se il valore di first_op e' minore di quello di second_op faccio vicinato come per GREATER_OR_EQUAL_OP
  ...
  if (start == 1 && level != -1)
    {
      for (i = 0; i < gnum_ef_conn; i++)
	{
	  if (gef_conn[i].level < 0 && GpG.force_neighbour_insertion!=TRUE )
	    {
	      if (DEBUG5)
		{
		  printf("\nWarning: trying to insert a non reachable action in neighborhood!! %s", print_op_name_string(i, temp_name));
		}
	      continue;
	    }
  ...
  **/
  ...
  for (indlevel = get_prev(level); indlevel >= 0; indlevel = get_prev(indlevel)) {
    ...
    switch (gcomp_var[fct_pos].l_operator) 
      ...
      case EQUAL_OP:
	// MODIFICATO X UGUALIANZA
	// se l'azione modifica il valore della variabile piu' di MAX_APPROX allora la metto nel vicinato x rimozione
...
constraints_list choose_random_inconsistence()
{
  /* Questa funzione seleziona una inconsistenza in modo casuale da uno dei
     ...
     La probabilita' di selezione di uno dei tipi viene fatta variare  
  ...
  **/
  for (i=0;i<GpG.num_false_num_fa;i++)
    ...
    **/
    ...
    /**
       Una volta trovate le inconsistenze presenti al livello piu' basso, casualmente se ne seleziona una
       **
 ...
 * Scopo: Questa funzione seleziona una inconsistenza scegliendone una casuale tra quelle che possiedono  il costo piu' basso
 * Tipo: constraints_list
 * Input:
 * Output:
 * Strutture dati principali:
...
constraints_list choose_min_level_cost_inconsistence()
{
  ...
  **/
  for (i=0;i<GpG.num_false_act;i++)
    ...
    /**
       Una volta trovate le inconsistenze presenti al livello piu' basso, casualmente si seleziona la meno costosa
       **
...
constraints_list choose_min_level_constr_inconsistence()
{
  ...
  **/
  for (i=0;i<GpG.num_false_act;i++)
    ...
    /**
       Una volta trovate le inconsistenze presenti al livello piu' basso, si seleziona la meno costosa
       **
       When the lower level inconsistence have been found, I select the less expensive
    **/
   /**
...
int define_neighborhood_for_fix(constraints_list unsup_fact, float *new_time)
{
  ...
  if(*unsup_fact->level>=GpG.curr_plan_length)
    {
      GpG.last_goal_max_action_level=get_prev(GpG.curr_plan_length);
    }
...
int search_step(int num_try, int min_time, int numrestart, PlanAction ** plan_actions, State * start_state, State * end_state)
{
  ...
  /** 
      ...
      state trovate gia' altre soluzioni.
  ...
  **/
  ...
  if (GpG.is_domain_numeric && (goptimization_exp >= 0))
      ...
      if (GpG.temporal_plan && GpG.end_time_action_f != NULL)
	{
	  ...
	  if(GpG.num_false_tot == 0 )
	    update_end_time_action_f(); // Additional check
  ...
  **/
  ...
  do
      ...
      for (num_restart = 0; num_restart < GpG.numrestart; num_restart++)
	{
	  ...
	  /**
	     Questo e' il numero totale dei fatti e delle azioni che erano false
	     **
