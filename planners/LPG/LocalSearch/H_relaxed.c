...
insert_action_inlist (int pos, int fact)
{
 ...
 /* ADAPT */
  ...
  if(Hvar.constr!=NULL)
    level=*Hvar.constr->level;
  else
    level=0;
  
  Hvar.cost_actions_define_cost += get_action_cost (pos, level, NULL);
  ...
  if (prec_par)
    {
	  ...
	  for (i = 0, temp = 0.0; i < gef_conn[act_pos].num_PC; i++)
		...
		{ // IIII ADD
		  ...
		  if (Hvar.constr && Hvar.constr->constraint_type == C_T_UNSUP_FACT
		      && el== Hvar.constr->fact)
			{
			  n_cost->weight= MAX_COST;
			  n_cost->act_cost= MAX_COST;
      ...
      if(gef_conn[neighb_act->act_pos].is_numeric && next_level<=GpG.curr_plan_length)
	{
	  ...
	  for(next_level=level+1;next_level<=GpG.curr_plan_length;next_level++)
		...
		continue;	 
	      ...
	      int tot_num_precs=gef_conn[action_pos].num_PC;
	      if(gef_conn[action_pos].sf!=NULL)
		{
		  tot_num_precs+=gef_conn[action_pos].sf->num_PC_overall;
		  tot_num_precs+=gef_conn[action_pos].sf->num_PC_end;

		}

	      /* Se il livello non e' vuoto e l'azione  e' numerica*/
	      for(counter=0;counter<tot_num_precs;counter++)
		{
		  int effect_indx=0;
		  if(counter<gef_conn[action_pos].num_PC)
		    effect_indx=gef_conn[action_pos].PC[counter];
		  else
		    if(counter<(gef_conn[action_pos].num_PC+gef_conn[action_pos].sf->num_PC_overall))
		       effect_indx=gef_conn[action_pos].sf->PC_overall[counter-gef_conn[action_pos].num_PC];
		    else
		      effect_indx=gef_conn[action_pos].sf->PC_end[counter-gef_conn[action_pos].num_PC-gef_conn[action_pos].sf->num_PC_overall];
		  // III TODO: update at_start effects


		  if(effect_indx<0)
		    {
		      if(!GET_BIT(Hvar.to_control,-(effect_indx)))
			continue;
		      if(vectlevel[next_level]->numeric->values[-(effect_indx)]<0.5)
			{
			  /* Se e' falso controllo se si e' allontanato ulteriormente 
			     dal valore di verita'*/
			  if(gcomp_var[-(effect_indx)].l_operator==GREATER_THAN_OP || 
			     gcomp_var[-(effect_indx)].l_operator== GREATER_OR_EQUAL_OP)
			    if(vectlevel[next_level]->numeric->values[gcomp_var[-(effect_indx)].first_op] >
			       Hvar.temp_num_level[gcomp_var[-(effect_indx)].first_op])
			      {
				/*In tal caso considero il costo per renderlo supportato*/
				unsat_facts++;
			       
				//	RESET_BIT(Hvar.to_control,-(effect_indx));
				temp_max= Hvar.common_max_values[gcomp_var[-(effect_indx)].first_op];
				temp_min=Hvar.common_min_values[gcomp_var[-(effect_indx)].first_op];
				Hvar.common_max_values[gcomp_var[-(effect_indx)].first_op]=Hvar.temp_num_level[gcomp_var[-(effect_indx)].first_op];
				Hvar.common_min_values[gcomp_var[-(effect_indx)].first_op]=Hvar.temp_num_level[gcomp_var[-(effect_indx)].first_op]; 
				/*
				...
				*/
				...
				temp= compute_relaxed_fact_cost (effect_indx,level, &loc_n_cost, next_level, MAXFLOAT);
			
				Hvar.common_max_values[gcomp_var[-(effect_indx)].first_op]=temp_max;
				Hvar.common_min_values[gcomp_var[-(effect_indx)].first_op]=temp_min;
			  ...
			  if(gcomp_var[-(effect_indx)].l_operator==LESS_THAN_OP || 
			     gcomp_var[-(effect_indx)].l_operator== LESS_THAN_OR_EQUAL_OP)
			    if(vectlevel[next_level]->numeric->values[gcomp_var[-(effect_indx)].first_op] <
			       Hvar.temp_num_level[gcomp_var[-(effect_indx)].first_op])
			      {
		      ...
		      float eval_prec=ri_eval_comp_var (&gcomp_var[-(effect_indx)], -(effect_indx) ,Hvar.temp_num_level,Hvar.temp_num_level,TRUE);
		      if(DEBUG5)
			{
			  printf("\nCheck act %d -- level %d, prec %s, val %f",action_pos, next_level,print_ft_name_string(effect_indx,temp_name),eval_prec);
		  ...
		  if(effect_indx<0)
		      ...
		      if(vectlevel[next_level]->numeric->values[-(effect_indx)]<0.5)
			{
			  ...
			  if(gcomp_var[-(effect_indx)].l_operator==LESS_THAN_OP || 
				...
				temp_max= Hvar.common_max_values[gcomp_var[-(effect_indx)].first_op];
				temp_min=Hvar.common_min_values[gcomp_var[-(effect_indx)].first_op];
				Hvar.common_max_values[gcomp_var[-(effect_indx)].first_op]=Hvar.temp_num_level[gcomp_var[i].first_op];
				Hvar.common_min_values[gcomp_var[-(effect_indx)].first_op]=Hvar.temp_num_level[gcomp_var[i].first_op];
		      ...
		      /* Se prima era vero ed adesso e' falso*/
		      if(ri_eval_comp_var (&gcomp_var[-(effect_indx)], -(effect_indx) ,Hvar.temp_num_level
					   ,Hvar.temp_num_level,TRUE)<0.5)
			{
...
dg_action_cost (neighb_list neighb_act)
{
  ...
  register EfConn *act;
  float total, prec_par, excl_par, add_effect_par, temp = 0.0, prec = 0.0, eff;
  FctNode_list inf_fact;
  int el = 0, fact_pos, ind_level;
  float precond, mutex, effect, act_cost;	//  LM  
 ...
 /* ADAPT */
  ...
  if (neighb_act->constraint_type == C_T_INSERT_ACTION)
      ...
      if (DEBUG4)
	{
	  if (DEBUG5)
	    printf ("\n\nEvalutate action");
	  printf ("\nDG_ACTION Act Insert: %s, level %d\n  ",
  ...
  if( GpG.consider_relaxed_plan_for_inconsistences==TRUE )
      ...
      if (DEBUG4)
	{
	  if (DEBUG5)
	    printf ("\n\nEvalutate action");
	  printf ("\nDG_ACTION Act Remove: %s, level %d\n  ",
		  print_op_name_string (act->position, temp_name), level);
...
double evaluate_level_for_goals(int level, State *goals)
{
  double temp, heuristic;
  node_cost loc_n_cost;
  ActNode_list act;
  int action_pos;
  int newlevel= GpG.curr_plan_length;
 /** 
      copio i valori del livello di action
      **
  ...
  **/
  ...
  Hvar.action = NULL;
  act = GET_ACTION_OF_LEVEL(level);
  action_pos= GET_ACTION_POSITION_OF_LEVEL(level);
  ...
  for(int i=0; i < goals->num_F; i++){
    ...
    loc_n_cost.act_time = 0.0;
    loc_n_cost.action=action_pos;
    local_search.ls_continue=1;
    
    temp=compute_relaxed_fact_cost (el, newlevel, &loc_n_cost, newlevel,  get_action_time (act->position, level));
    
  }

  heuristic= Hvar.num_actions_define_cost + Hvar.weight_facts_define_cost;
  return heuristic;
