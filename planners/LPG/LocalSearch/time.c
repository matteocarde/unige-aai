 ...
 * Output: 1 se l'inserimento e' andato a buon fine
 *         0 se l'azione e' gia' presente nella propagation list
 *         un messaggio di errore se l'azione non pu� essere inserita
...
int insert_propagation_list (ActNode_list infAction)
{
  ...
  **/          
  ...
  /**
     ...
     Se non e' presente la inserisci e ritorna 1 altrimenti non 
  ...
  for (j = 0; j < gef_conn[infAction->position].num_A; j++)
	 ...
	 **
      ...
      **/
	  ...
	  add_effect->time_f == NOTIME|| (CONVERT_FACT_TO_NODE (cel, level ))->w_is_true==0)
	  {
  ...
  for (ind = level + 1; ind < GpG.curr_plan_length; ind++)
	 ...
	 **
      ...
      **/
      ...
      //  propagation_time (GET_ACTION_OF_LEVEL (ind),GpG.);
...
void insert_time (ActNode_list infAction)
{
  ...
  **/
  ...
  /**
     Precondizioni AT_START. Se la precondizione relativa all'azione e'
     falsa prendo l'istante minimo a cui la precondizione e'
     raggiungibile mentre se e' vera prendo l'istante in cui la
     precondizione e' supportata.
  **
	  ...
	  if (infEl->w_is_true < 1)
	    {
          ...
          /**
	     Se il fatto e' vero prendiamo l'istante effettivo a cui
	     e' supportato.  Da notare che siccome e' una precondizione
...
void remove_temporal_constraints (int posAction)
{
  ...
  * permette tali associazioni e la prima precondizione che e' stata
...
float find_temporal_interval(float t, ActNode_list infact, int *first_moved) {
  ...
  // Alloco il vettore per gli intervalli se nn e' gia' stato fatto
...
/**
 * Controlla il se time assegnato all'azione infact e' consistente o meno con gli
...
float check_time_interval(float tprec, ActNode_list infact) {
  ...
  // Se non ho modificato il tempo e tprec e' inconsistente
  if (time < 0) {
    /*
  ...
  // Se non ho modificato l'intervallo e tprec e' consistente
  else if (time == start) {
  ...
  // Se l'istante start era inconsistente e sono passato ad un intervallo successivo 
    ...
    /*
      ...
      // Segnalo un inconsistenza sulla prima precondizione timed che e' stata spostata
      if (first_moved >= 0) {
  ...
  // Se l'azione pu� essere spostata in un intervallo precedente
